 ...
 * 璀璨宝石房间等待页
 *
 * 房间号显示+复制，玩家列表，准备/开始，添加/移除人机（2-4人）
 */
...
export default function SplendorRoom() {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-emerald-600 via-teal-700 to-green-800 flex items-center justify-center p-4">
      <div className="w-full max-w-md bg-white/95 backdrop-blur rounded-2xl shadow-2xl p-6 space-y-5">
                ...
                >
                  <div className="flex items-center gap-2">
                    {player.isAI
                      ...
                      : <UserRound className={`size-5 ${isSelf ? 'text-emerald-500' : 'text-gray-400'}`} />
                    }
                    <span className={`font-medium ${isSelf ? 'text-emerald-700' : 'text-gray-800'}`}>
                      {player.name}
                      {isSelf && <span className="text-xs text-emerald-400 ml-1">（我）</span>}
                    </span>
        ...
        <div className="space-y-2">
          {isHost ? (
            <Button className="w-full h-11 bg-emerald-600 hover:bg-emerald-700" disabled={!canStart} onClick={startGame}>
          ...
          <Button
            ...
            onClick={() => { leaveRoom(); window.location.href = '/splendor' }}
          >
