...
export default function HalliRoom() {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-yellow-400 via-orange-500 to-red-500 flex items-center justify-center p-4">
      <div className="w-full max-w-md bg-white/95 backdrop-blur rounded-2xl shadow-2xl p-6 space-y-5">
                ...
                >
                  <div className="flex items-center gap-2">
                    {player.isAI
                      ...
                      : <UserRound className={`size-5 ${isSelf ? 'text-orange-500' : 'text-gray-400'}`} />
                    }
                    <span className={`font-medium ${isSelf ? 'text-orange-700' : 'text-gray-800'}`}>
                      {player.name}
                      {isSelf && <span className="text-xs text-orange-400 ml-1">（我）</span>}
                    </span>
        ...
        <div className="space-y-2">
          {isHost ? (
            <Button className="w-full h-11" disabled={!canStart} onClick={startGame}>
          ...
          <Button
            ...
            onClick={() => { leaveRoom(); window.location.href = '/halli' }}
          >
