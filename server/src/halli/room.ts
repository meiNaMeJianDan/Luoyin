 ...
 * 以及 AI 玩家管理。
 * 参考 UNO 和 Catan 房间管理模块的实现模式。
 */

import { v4 as uuidv4 } from 'uuid';
import type { HalliPlayer, HalliRoom, HalliRoomStatus } from './types.js';
...
/** 所有房间，key 为 6 位数字房间号 */
const rooms: Map<string, HalliRoom> = new Map();
...
export function createRoom(
  playerName: string,
  socketId: string,
): { room: HalliRoom; playerId: string } | { error: string } {
 ...
 */
export function leaveRoom(roomId: string, playerId: string): HalliRoom | null {
  const room = rooms.get(roomId);
  if (!room) return null;
 ...
 * 切换准备状态
 */
export function toggleReady(roomId: string, playerId: string): HalliRoom | null {
  const room = rooms.get(roomId);
  if (!room) return null;
 ...
 */
export function canStartGame(room: HalliRoom): boolean {
 ...
 */
export function addAIPlayer(roomId: string): HalliRoom | { error: string } {
  const room = rooms.get(roomId);
 ...
 */
export function removeAIPlayer(roomId: string): HalliRoom | { error: string } {
  const room = rooms.get(roomId);
  if (!room) return { error: '房间不存在' };
  if (room.status !== 'waiting') return { error: '游戏已开始' };
 ...
 * 获取房间信息
 */
export function getRoom(roomId: string): HalliRoom | undefined {
 ...
 * 更新房间状态
 */
export function setRoomStatus(roomId: string, status: HalliRoomStatus): void {
  const room = rooms.get(roomId);
  if (room) {
    room.status = status;
  }
 ...
 * 切换房间的 AI 托管开关
 */
export function toggleAllowAI(roomId: string): HalliRoom | null {
  const room = rooms.get(roomId);
  if (!room) return null;
 ...
 * 获取所有房间（调试用）
 */
export function getAllRooms(): Map<string, HalliRoom> {
  return rooms;
}
 ...
 * - 取消 AI 托管状态（仅对原本不是 AI 的玩家，即真人掉线后被 AI 托管的情况）
...
export function handleReconnect(
  ...
  // 取消 AI 托管（仅对有 socketId 记录的真人玩家，原生 AI 玩家 socketId 为空字符串）
  // 原生 AI 玩家不会触发重连，所以这里直接设为 false 即可
  player.isAI = false;
 ...
 * - 遍历 socketToPlayer 映射找到对应的房间和玩家对象
 * - 返回完整的 room 和 player 对象，找不到返回 null
 */
export function findPlayerBySocketId(socketId: string): { room: HalliRoom; player: HalliPlayer } | null {
  const mapping = socketToPlayer.get(socketId);
  if (!mapping) return null;
